...
from main.config import load_db, load_index, load_model
...
def get_files(file_path):
    ...
    for f in files:
        print(f"\n \n This is the file: {f} \n \n")
        file = f"{file_path}/{f}"
        run_bm25_indexing(conn, cur, file)
...
def run_ingestion(model, index, conn, cur, file):
    ...
    for page_num in tqdm(range(num_pages), desc=f"Extracting {os.path.basename(file)}"):
        ...
        # cool feature of python, where you can pass in an array and it appends induvidual elements!
